...
Evaluate the original aggregate demand and the supply curve at the new equilibrium quantity determined in 6(a)  to determine the marginal benefit and the marginal cost at Q = `r round(Q_eq_e, 0)` gal. The tax to limit environmental damage is the difference between marginal benefit and marginal cost curves at the quantity, Q~e, eq~. 

Aggregate Demand = `r round(m_agg, 6)`*Q + `r round(int_agg,2)`
Aggregate Supply = `r round(m_MC, 6)`*Q 
...
```
P~d~(Q~e,eq~)= \$`r round(D_agg_q_eq_e,2)`
P~s~(Q~e,eq~)= \$`r round(S_q_eq_e,2)`
