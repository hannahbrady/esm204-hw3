...
```
The new price of gasoline is found by solving the marginal social cost curve for P~eq~ given the new equilibrium quantity calculated in Part 4.b above:

  Price~[socially optimal]~ = `r round(m_MC,6)`*`r round(MSC_Q_eq,1)` + `r round(MEC,2)`= \$ `r round(MSC_P_eq,2)` 
...
+ Solve D~high~ = `r round(m_high,6)`Q + `r round(int_high,2)` at the new socially optimum equilibrium price \$`r round(MSC_P_eq, 2)`/gal
...
+ Integrate "high" demand curve (`r round(m_high, 6)`Q + `r round(int_high, 2)`) from 0 to `r round(MSC_Q_eq_high, 2)` then subtract the cost of purchasing gas (\$`r round(MSC_P_eq, 2)`/gal*`r round(MSC_Q_eq_high, 2)` gal)
    ...
    Welfare~[SO high]~ = \$`r round(CS_high_MSC_total,1)` 

#### d. Overall welfare of "low" income consumers

Calculate the welfare to the low income group by determining the consumer surplus at the new equilibrium and subtracting the cost of purchasing gas and total environmental cost of consuming gas at the new socially optimal equilibrium
...
+ Solve D~low~ = `r round(m_low,6)`Q + `r round(int_low,2)` at the new socially optimum equilibrium price \$`r round(MSC_P_eq, 2)`/gal

    Q~SO~ = `r round(MSC_Q_eq_low, 2)`gallons

+ Integrate "low" demand curve (`r round(m_low, 6)`Q + `r round(int_low, 2)`) from 0 to `r round(MSC_Q_eq_low, 2)` then subtract the cost of purchasing gas (\$`r round(MSC_P_eq,2)`/gal*`r round(MSC_Q_eq_low, 2)` gal)
...
Find the welfare of the producers by finding the total income generated at the socially optimal equilibrium and subtracting the cost of production. 

+ Income = P~SO~*Q~SO~
+ Cost: Integrate MPC curve (`r round(m_MC, 6)`Q) from 0 to Q~SO~ (`MSC_Q_eq` gal)
+ Welfare: Income - Cost
...
Tax revenue is found by multiplying the MEC (\$ `r round(MEC, 2)`per gallon) by the quantity consumed Q~SO~ (`r round(MSC_Q_eq,2)`)gallons

Tax Revenue= \$ `r round(TED,2)` 

# Part 5

*Now, assume that all revenue from the gas tax will be redistributed to the consumers in proportion to their pre-tax consumption of gas. For example, if 80% of the gas was consumed by “high” income consumers, then they get 80% of the tax revenue. Additionally, consider the fact that current scientific evidence suggests the true SCC may be much higher than $51. For a range of SCC values ($51, $75, $100, $125, and $150 per metric ton of CO2), calculate the effects of an SCC-based gasoline tax on:*

Taxes are redistributed proportional to original consumption of gas between high and low income groups at the free market equilibrium price.
...
```
Welfare~$51~= \$`r round(wf_high_MSC, 2)`

Welfare~$75~= \$`r round(wf_high_MSC_75, 2)`

Welfare~$100~= \$`r round(wf_high_MSC_100, 2)`

Welfare~$125~= \$`r round(wf_high_MSC_125, 2)`

Welfare~$150~= \$`r round(wf_high_MSC_150, 2)`
...
```
Welfare~$51~= \$`r round(wf_low_MSC, 2)`

Welfare~$75~= \$`r round(wf_low_MSC_75, 2)`

Welfare~$100~= \$`r round(wf_low_MSC_100, 2)`

Welfare~$125~= \$`r round(wf_low_MSC_125, 2)`
...
#Cost of production
#Area under the mc curve from 0 to the quantity produced, Q (gal)
cost <- integrate (MC, lower = 0, upper = MSC_Q_eq)
...
part_5_final <- part_5_table %>% 
  select(SCC, TED, welfare_high, welfare_low, gas_rev)

part_5_final %>% 
  kable(digits = 0, format.args = list(big.mark = ","), 
    caption = "Caption",
    col.names = c("SCC", "Total Environmental Damage", "Welfare to high demanders", "Welfare to low demanders","Welfare to gas producers")) %>% 
