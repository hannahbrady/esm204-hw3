...
+ $51/ton CO2~~ results in an MEC/SCC-based gasoline tax of \$`r round(MEC,2)`/gal
...
Solve for a new y-intercept for the demand curve by reducing the previous intercept (P=`r round(int_high,2)`) by half. The slope for the new demand curve will remain the same (m= `r round(m_high,6)`) because the total demand for gasoline has just shifted. 
...
The resulting new high income demand curve is P=`r round(m_high,6)`*Q-`r round(int_high_e,2)` 

Use the new high income demand curve to generate an aggregate demand curve by summing individual demand curves. Solve each equation for Q, add both equations together, and re-solve for P. The aggregate demand equals the demand for the low income consumer group at prices where only the low demand consumer group has demand: 
...
The resulting aggregate demand curve when electric technology is introduced is P=`r round(m_agg,6)`*Q+`r round(int_agg_e,2)`
...
The new equilibrium quantity of gas is found by setting the aggregate demand curve with electric technology equal to the supply curve and solving for . 
...
Q~eq,e~= `r round(Q_eq_e, 2)` gallons
...
Total environmental damage is calculated by multiplying the marginal external cost (MEC= \$`r round(MEC, 2)`) calculated in section 2 by the new equilibrium quantity of gas sold (Q=`r round(Q_eq_e, 2)` gal)
