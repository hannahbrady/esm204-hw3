...
<<<<<<< HEAD
TED= `r round(MEC, 2)`*Q when Q= `r round(MSC_Q_eq, 2)` gallons
=======
TED = `r round(MEC, 2)`*Q, Q~SO~ = `r round(MSC_Q_eq, 2)`
>>>>>>> f59e5ad941be654de65d06e70ae56c72af4de8ce
...
<<<<<<< HEAD
Tax revenue is found by multiplying the MEC=\$ `r round(MEC, 2)` per gallon by the quantity consumed Q=`r round(MSC_Q_eq,2)`gallons
...
Taxes are redistributed proportional to origainal consumption of gas between high and low income groups at the free market equilibrium price.
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#MEC of one gallon of gas at different SCC
# MEC = ($/ metric ton) * (metric tons/lb) * (lb/gal)
...
#### c. Gas producers
=======
Tax revenue is found by multiplying the cost of the tax, the MEC (\$`r round(MEC, 2)`/gal) by the quantity consumed Q~SO~ =`r round(MSC_Q_eq,2)`
